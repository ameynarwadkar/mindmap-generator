...
class MindMapGenerator:
    ...
    async def generate_mindmap(self, document_content: str, request_id: str) -> str:
        ...
        try:
            ...
            try:
                ...
                return self._generate_markmap_html(verified_concepts)
                ...
                return self._generate_markmap_html(concepts)
    ...
    def _generate_markmap_html(self, concepts: Dict[str, Any]) -> str:
        """
        Build Markdown + a tiny HTML shell that the Markmap CDN converts
        into an interactive mind-map.  Returns the HTML string (useful if
        the caller wants it), and ALSO writes:
            • output.md
            • mindmap.html
        """
        # 1. Convert the same concept structure to Markdown
        central = concepts.get("central_theme", {})
        print("DBG - #topics:", len(central.get("subtopics", [])))
        md_text = self._build_markdown(central)

        if not central.get("subtopics"):
            raise ValueError("No sub-topics found in concepts['central_theme']")


        Path("output.md").write_text(md_text, encoding="utf-8")

        # 2. Wrap the Markdown in a minimal Markmap HTML shell
...
---
        ...
        Path("mindmap.html").write_text(html_shell, encoding="utf-8")
        return html_shell   # <-- optional; caller can ignore
    ...
    def _build_markdown(self, tree: dict) -> str:
        """
        Recursively walk *tree* and return a Markdown document whose headings
        are understood by Markmap as collapsible mind-map nodes.
        """
        md: list[str] = []
        self._add_md_line(md, 1, tree["name"])                    # root

        for topic in tree.get("subtopics", []):
            self._add_md_line(md, 2, topic["name"])               # depth 1
            for sub in topic.get("subtopics", []):
                self._add_md_line(md, 3, sub["name"])             # depth 2
                for detail in sub.get("details", []):
                    self._add_md_line(md, 4, detail["text"])              # depth 3
...
# # ======================================================================
# # QUICK SANITY-CHECK  ──────────────────────────────────────────────────
# # Run   python mindmap_generator.py --smoke   to test Markmap pipeline
# # ======================================================================
# if __name__ == "__main__":
...
#     # ------------------------------------------------------------
#     #  Build a FAKE concept tree that mimics the real structure
#     # ------------------------------------------------------------
#     fake = {
#         "central_theme": {
...
#     gen = MindMapGenerator()          # use your real class name if different
#     html = gen._generate_markmap_html(fake)
...
#     print("------- output.md -------")
#     print(md)
#     print("------- mindmap.html (first 15 lines) -------")
#     print("\n".join(html.splitlines()[:15]))
#     print("Smoke-test complete → open mindmap.html in your browser.")
